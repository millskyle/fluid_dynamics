...
\end{align*}
\Figure{fig/ass6/A6_Q1_a_plot.eps} shows a two-dimensional plot of this flow.  The fluid is as expected, travelling the fastest at the midpoint between the boundaries and is not moving at the boundary, consistent with the no slip condition.\\
...
Recall that the differential equation  we're trying to solve is Laplace's equation, \Equation{laplacesequationforwaves}.  The problem is that we really have two unknown functions: $\phi$ and $\eta$.  We can't obtain both, so we must make an assumption; we'll assume that our  wave is a travelling sinusoidal wave of the form
...
In the previous section, we assumed that a wave was perfectly sinusoidal, but that's not very realistic for many disturbances.  It might be a good approximation for waves in deep water in the middle of the ocean, but if we throw a rock into a pond, the resulting disturbance is definitely not going to be a perfect sine function.  We need to talk about a more generalized form of \Equation{travellingwaveeta}.  Our goal of this section will be to build up a mathematical representation of a generic disturbance that isn't limited to being a perfect sinusoidal function.  For this, the concept of a Fourier Series will come in handy: \textit{Any periodic function $f(x)$ can be written as a function of sines and cosines:}
\numeq{ f(x) = \frac{a_0}{2} + \sum\limits_{n=1}^{\infty} \left[ a_n\cos(nx) + b_n\sin(nx) \right].}{23423412664684674643341}
The Fourier series, rather than encoding information as a function of $x$, really encodes the information in the real-valued coefficients $a_n$ and $b_n$.  We can also write the function a bit more concisely as
\numeq{f(x) = \sum\limits_{n=-\infty}^{\infty} c_n e^{inx}}{423947019237401928364701982763409813}
where $c_n$ are complex-valued coefficients.  It is usually much easier to work with exponentials than it is sines and cosines, even with complex numbers,  so we will elect to work with the latter form.




\Example{289736419283469182364}{We'll begin with an idealized example which will demonstrate just one way that a more complicated wave can constructed.  Consider two sinusoidal waves added together:
...
The second cosine term in \Equation{9023874109238740192634091236740912} corresponds to the normal travelling wave which we've worked with already. The first cosine out front, however is something we haven't seen before. It acts as an envelope for the travelling wave sandwiching the normal travelling wave within the boundary of the first cosine function, shown in \Figure{ch6_wave_packet}.  The amplitude is \textit{modulated}.  The idea to take away from this is that when you add two cosine waves together, you get a much more complicated wave. The concept of amplitude modulation is central to the Fourier techniques that we'll now proceed with.
}
\inkscapeimage{ch6_wave_packet}{0.5}
...
\numeq{f(x) = \frac{1}{2\pi}\int_{-\infty}^\infty e^{-i\omega x}d\omega \int_{-\infty}^\infty f(t)e^{i\omega t}dt. }{FourierIntegralWaves}
We'll break this up into two separate functions: first we'll  define
\unnumeq{g(\omega) = \frac{1}{\sqrt{2\pi}}\int^\infty_{-\infty}f(t)e^{i\omega t} dt}
leaving
\unnumeq{f(x) = \frac{1}{\sqrt{2\pi}} \int^\infty_{-\infty}  g(\omega)e^{-i\omega x}d\omega }


The best way to demonstrate this is through a generic example:

\Example{FourierIntegralExample}{Consider the finite sine wave
\unnumeq{
f(x) = \left\{
     \begin{array}{cr}
       \sin(\omega_0 x), & |x| < n\pi/\omega_0 \\
       0 & \mathrm{otherwise}
     \end{array}
     \right.
}
